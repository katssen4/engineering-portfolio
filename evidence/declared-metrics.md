...
| 32,159 lines, 126 Python files | the development harness, of which `evidence/agent-governance/` ships a reduced extract | `git ls-files <harness dir> \| grep -E '\.py$' \| tr '\n' '\0' \| xargs -0 cat \| wc -l` | 32159 | 2026-09-11 |
...
appears as `<exclusions>`, the eight products are `same command per project, summed`, and the
English corpus is `then the same for en/`. Those are recipes, not commands you could paste. The
